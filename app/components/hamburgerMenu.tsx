...
export default function HamburgerMenu({menuOpen}:HamburgerMenuProps) {
    ...
    return (
        <div 
          id="hamburger-menu" 
          className="fixed top-0 w-full h-screen bg-neutral-50 z-30 transition-all"
        ...
        >
            ...
            <ul className="flex flex-col px-4 py-40">
              {
                links.map((link,i) => (
                  <Link key={i} href={link.href} className="flex items-center hamburger-links gap-4 text-3xl">
